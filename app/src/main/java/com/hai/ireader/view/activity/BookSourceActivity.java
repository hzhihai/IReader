...
import com.hai.ireader.presenter.BookSourcePresenter;
import com.hai.ireader.presenter.contract.BookSourceContract;
import com.hai.ireader.service.BookSourceService;
import com.hai.ireader.service.ShareService;
...
import com.hai.ireader.view.adapter.BookSourceAdapter;
...
public class BookSourceActivity extends MBaseActivity<BookSourceContract.Presenter> implements BookSourceContract.View {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case R.id.action_add_book_source: //新建书源
                addBookSource();
                break;
            case R.id.action_select_all: //选中所有或不选
                selectAllDataS();
                break;
            case R.id.action_import_book_source_local://本地导入书源
                selectBookSourceFile();
                break;
            case R.id.action_import_book_source_onLine: //网络导入书源
                importBookSourceOnLine();
                break;
            case R.id.action_import_book_source_rwm: //二维码导入
                scanBookSource();
                break;
            case R.id.action_revert_selection://反转选择
                revertSelection();
                break;
            case R.id.action_del_select://删除所选
                deleteDialog();
                break;
            case R.id.action_check_book_source: //校验所选
            ...
            case R.id.sort_pin_yin:
                upSourceSort(2);
                break;
            case R.id.action_share_wifi: //wifi分享
     ...
     * @param cacheUrl
     */
    private void updateBookSource(String url,String cacheUrl){
        if (cacheUrl == null) {
            ACache.get(this).put("sourceUrl", url);
            mPresenter.importBookSource(url);
        }else if ( url == ""){
            return;
        }
        else {
    ...
    private void importBookSourceOnLine() {
        ...
        InputDialog.builder(this)
                ...
                .setCallback(inputText -> {
                    ...
                    updateBookSource(inputText,cacheUrl);
                }).show();
